...
int main(void)
{
	...
	if (!device_is_ready(lsm6dsl_dev)) {
		printk("Gyroscope device not ready\n");
		return 0;
	}
...
static void gyro_thread(void *p1, void *p2, void *p3)
{
	...
	const struct device *lsm6dsl_dev = DEVICE_DT_GET_ONE(st_lsm6dsl);
	double gyro_x_offset = 0.012538;
	double gyro_y_offset = -0.051161;
	double gyro_z_offset = 0.004459;
	...
	if (sensor_trigger_set(lsm6dsl_dev, &trig, lsm6dsl_trigger_handler) != 0) {
		printk("Could not set gyroscope trigger\n");
		return;
	}

	while (true) {
		struct mouse_pos pos;
		pos.x_val = (int16_t)(-10 * (sensor_value_to_double(&gyro_x_out) - gyro_x_offset));
		pos.y_val = (int16_t)(10 * (sensor_value_to_double(&gyro_y_out) - gyro_y_offset));
...
K_THREAD_DEFINE(gyro_thread_id, 1024, gyro_thread, NULL, NULL, NULL, 5, 0, 0);
